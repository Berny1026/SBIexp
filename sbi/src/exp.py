...
def grad_sphere(x, y, z):#求球面上点的梯度
    grad_x = 2*x
    grad_y = 2*y
    grad_z = 2*z
    grad = np.array([grad_x, grad_y, grad_z])
...
def grad_level_set(point):#求等值集的梯度
    x = point[0]
    y = point[1]
    z = point[2]
    if surface == 'torus2':
...
def to_id(id_x, id_y, id_z, base):#疑问：不知道这样一个计算是用来干嘛的
    return id_x * np.power(base, 2) + id_y * base + id_z

def get_vertices(id_x, id_y, id_z, h):#用这个函数找到顶点，
    vertices = []
